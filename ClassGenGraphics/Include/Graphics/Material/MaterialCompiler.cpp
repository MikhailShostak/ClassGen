...
void MaterialCompiler::Compile(const SharedReference<ClassGen::BaseInfo>& BaseInfo, const System::Path& OutputFilePath)
{
    auto t = DynamicCast<ClassGen::MaterialInfo>(BaseInfo);
    if (!t)
    {
        return;
    }
