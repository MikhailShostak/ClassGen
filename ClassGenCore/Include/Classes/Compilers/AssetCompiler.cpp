...
void AssetCompiler::Compile(const SharedReference<ClassGen::BaseInfo>& BaseInfo, const System::Path& OutputFilePath)
{
    auto a = DynamicCast<ClassGen::AssetInfo>(BaseInfo);
    if (!a)
    {
        return;
    }
    ...
    auto assetNamespace = boost::replace_all_copy(assetInfo.Namespace, ".", "::");
    auto assetBaseType = writeRecursivelyResolved(assetInfo.BaseType);
