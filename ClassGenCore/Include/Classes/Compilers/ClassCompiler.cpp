...
void ClassCompiler::Compile(const SharedReference<ClassGen::BaseInfo>& BaseInfo, const System::Path& OutputFilePath)
{
    ...
    file << "struct PUBLIC_API_EXPORT " << className << "\n";
    ...
    for (const auto& p : classInfo.Properties)
    {
        ...
        if (!p.Initialization.empty())
        {
            file << " { " << p.Initialization << " };\n";
        }
        else
        {
            if (ranges::contains(p.Attributes, "Incomplete"))
            {
                file << ";\n";
            }
            else
    ...
    auto addReference = [&](const ClassGen::ParameterInfo &p, const String& name) { return p.Copy ? name : (name + " &"); };
    auto addConst = [&](const ClassGen::ParameterInfo &p, const String& name) { return p.Writable ? name : ("const " + name); };
    for (const ClassGen::FunctionInfo& f : classInfo.Functions)
    {
        Array<String> parameters = f.InputParameters | ranges::views::transform([&](auto& p)
            {
                return addConst(p, addReference(p, writeRecursivelyResolved(p.Type))) + " " + p.Name;
            }) | ranges::to<Array<String>>();
            ...
            auto resolveReturnType = [&]() -> String
            {
                ...
                Array<String> parameters = f.OutputParameters | ranges::views::transform([&](auto& p) { return addConst(p, addReference(p, writeRecursivelyResolved(p.Type))) + "/*" + p.Name + "*/"; }) | ranges::to<Array<String>>();
